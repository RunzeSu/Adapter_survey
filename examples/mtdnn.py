...
import tokenization
from modeling_grouped import BertConfig, BertForSequenceClassification, BertForMultiTask
...
def main():
        ...
        if args.multi:
            ...
            for n, p in model_dict.items():
                if 'aug' in n or 'mult' in n:
                    ...
                    if ('_list' in n) or ('task_emb' in n) or ('_group' in n):
    ...
    if args.do_train:
        ...
        for i, task in enumerate(task_names):
            ...
            print("***** training data for %s *****", task)
            print("  Data size = %d", len(train_features))
        ...
        print("  Num param = {}".format(total_params))
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step in range(steps_per_epoch):
                ...
                task_id = task_id % num_tasks
                #print(num_tasks, task_id)
                batch = next(loaders[task_id])
                batch = tuple(t.to(device) for t in batch)
                input_ids, input_mask, segment_ids, label_ids = batch
                """    
                #Freeze partial bert layers            
                if epoch < 20: 
                ...
                if step % 1000 < num_tasks:
                    print("Task: {}, Step: {}".format(task_id, step))
                    print("Loss: {}".format(tr_loss[task_id]/nb_tr_steps))
